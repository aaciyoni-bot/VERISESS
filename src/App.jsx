...
// ==========================================
// 2. ווידג'טים ומשחקים (הוטמעו כאן למניעת שגיאות)
// ==========================================
...
// ווידג'ט הפוקר המעודכן (כולל Rake וכסף מדומה)
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {[1, 2, 3].map((i) => (
        <div key={i} className="bg-white rounded-2xl p-6 shadow-md border border-gray-100 flex flex-col items-center text-center">
          <div className="w-24 h-24 bg-gradient-to-tr from-blue-100 to-teal-100 text-teal-600 rounded-full flex items-center justify-center font-bold text-3xl mb-4 shadow-sm">{i === 1 ? 'ד' : i === 2 ? 'ע' : 'ק'}</div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      <div className="bg-gray-900 text-gray-300 text-xs py-2 px-4 flex flex-wrap gap-4 items-center justify-center border-b-4 border-red-500" dir="rtl">
        ...
        <button onClick={() => { setRoomCategory('gaming'); setCurrentView('videoRoom'); }} className="text-amber-400 font-bold hover:text-amber-300 transition-colors">קזינו (Rake אמיתי)</button><span>|</span>
        <button onClick={() => { setRoomCategory('class'); setCurrentView('videoRoom'); }} className="text-blue-400 font-bold hover:text-blue-300 bg-blue-900/50 px-2 py-1 rounded">חוג (כסף מדומה + ציור)</button>
      </div>
