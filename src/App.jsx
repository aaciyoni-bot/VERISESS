...
const VideoRoom = ({ sessionId, onLeave, isProvider = true, category = 'gaming' }) => {
  ...
  const addTestParticipant = () => {
    setParticipants(prev => [
      ...prev, 
      { id: `p${prev.length}`, name: `שחקן ${prev.length}`, isLocal: false, img: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?ixlib=rb-4.0.3&w=1000&q=80' }
    ]);
  };
    ...
    <div className="max-w-7xl mx-auto my-4 bg-gray-900 rounded-2xl overflow-hidden shadow-2xl h-[85vh] flex relative border border-gray-800" dir="rtl">
      ...
      <div className="flex-1 relative bg-black flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 bg-gray-900/90 backdrop-blur-md px-8 py-3 rounded-full flex gap-4 z-30 border border-gray-700 shadow-2xl">
          ...
          <button onClick={() => toggleMedia('video')} className={`p-4 rounded-full transition-colors ${cameraEnabled ? 'bg-gray-700 text-white' : 'bg-red-500/20 text-red-500'}`}>
        ...
        <div className="flex flex-wrap bg-white border-b border-gray-200 p-1">
          ...
          {category === 'gaming' && (
            <>
              ...
              <button onClick={() => setActiveTab('rummikub')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-colors flex items-center justify-center gap-1.5 ${activeTab === 'rummikub' ? 'bg-blue-100 text-blue-700 border border-blue-200 shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}>
                <Dices className="w-4 h-4" /> רמיקוב
              </button>
              <button onClick={() => setActiveTab('chess')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-colors flex items-center justify-center gap-1.5 ${activeTab === 'chess' ? 'bg-gray-200 text-gray-800 border border-gray-300 shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}>
                <Gamepad2 className="w-4 h-4" /> שחמט
              </button>
            </>
          )}
        </div>
        ...
        <div className="flex-1 overflow-hidden p-3 bg-gray-50 flex flex-col">
          ...
          {(activeTab === 'rummikub' || activeTab === 'chess') && (
            <div className="h-full w-full p-6 bg-gray-100 rounded-xl flex flex-col items-center justify-center text-center border-2 border-dashed border-gray-300">
               ...
               <p className="text-sm text-gray-500">הלוח האינטראקטיבי יתווסף בהמשך.</p>
            </div>
          )}
...
export default function App() {
  return (
    <div className="bg-gray-100 min-h-screen font-sans p-4">
      <VideoRoom sessionId="poker_club_99" isProvider={true} category="gaming" />
    </div>
  );
}
